...
class PENDULUM:
    r"""Pendulum example, where the state lives on the 2-sphere.
    ...
    e3 = -np.array([0, 0, 1])
    r"third coordinate vector :math:`\mathbf{e}^b=-[0,0,1]^T`."

    H = np.zeros((2, 3))
    r"observability matrix :math:`\mathbf{H}`."
    H[:, 1:3] = np.eye(2)
