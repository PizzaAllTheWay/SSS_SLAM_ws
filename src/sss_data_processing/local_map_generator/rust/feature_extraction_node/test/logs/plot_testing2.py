...
def make_green_overlay(mask):
    overlay = np.zeros((mask.shape[0], mask.shape[1], 3), dtype=np.uint8)

    # Green = surviving semantic candidates after pair logic
...
def simple_semantic_threshold(image, relevant_min=50, local_window_size=51, local_offset=3):
    ...
    # Final green semantic mask / overlay.
    green_overlay = make_green_overlay(paired_mask)
...
def plot_semantic_pipeline(df, title="Relevant / Bright / Shadow / Semantic"):
    ...
    im4 = ax4.imshow(result["green_overlay"], interpolation="nearest")
    ax4.set_title("Green Semantic Mask")

    ax5.imshow(masked(image), cmap=cmap_img, interpolation="nearest", vmin=vmin, vmax=vmax)
    ax5.imshow(result["green_overlay"], interpolation="nearest", alpha=0.65)
    ax5.set_title("Green Overlay on Filtered")
    ...
    def update(val):
        ...
        im4.set_data(result["green_overlay"])
